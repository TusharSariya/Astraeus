...
An offline raw-to-artifact-to-reader replay validates retained inputs, compares
the rebuilt artifact with the retained artifact, writes regenerated evidence to
a separate output directory, and proves one complete selected lead (not a cycle
or horizon). It does not prove an existing production HTTP route.
