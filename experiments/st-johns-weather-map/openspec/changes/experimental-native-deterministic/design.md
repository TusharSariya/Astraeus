...
Indexed replay starts from the retained index and concatenated selected GRIB
messages, validates their recorded size and SHA-256 before decode, and compares
the rebuilt artifact with the retained artifact. Input and output directory
trees cannot overlap, and any offline integrity failure exits unsuccessfully.
ICON replay validates and decompresses each retained field and
...
Offline replay retains each original source retrieval timestamp in artifact and reader provenance; the summary records `replayed_at` separately. Missing original retrieval time refuses admitted artifact replay.
