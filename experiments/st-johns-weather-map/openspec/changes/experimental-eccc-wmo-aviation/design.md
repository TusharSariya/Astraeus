...
## Manual capture ceilings and identity
...
closed. These are individual request and parser refusal ceilings for manual
evidence capture. They are not an operation allocation or memory bound, so
these adapters remain ineligible for worker admission. Requests ask for
identity transfer encoding so retained payload bytes are not silently
transport-decoded.
...
the complete native path, QName, attribute-value, and text inventory are
retained without canonical mapping.
...
completion. A replay of those exact raw bytes and recorded completion receipts
retains full XML and line inventories plus artifact provenance in scratch. Its
committed compact receipt records hashes and counts for that replay. Raw
payloads and full inventories remain outside Git. This is
