...
Publish required companions as one atomic revision group through the existing immutable storage transaction. Route exactly one current revision through a dedicated reference-input resource endpoint; exclude these artifacts from `/point`, `/layers`, `/timeline`, consensus, nearest-time sampling, and every science path. Bound each revision group to 64 MiB before retrieval, retain at most eight revisions per source, and apply the concrete supersession and expiry rules in the design.
