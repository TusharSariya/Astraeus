# Design: Exhaustive source-to-task routing

The JSON roster is the executable planning index. Registered rows are generated
from the fixed audit inventory and enriched from `registry/source_data.py` so
field, level, coverage, authentication and licence claims remain inspectable.
Research rows preserve appendix row granularity plus every named narrative
group that would otherwise disappear from a table-only extraction.

One row can route a deliberately grouped family. Grouping is an accounting
...
The root worktree's uncommitted readiness-image specification change is a
