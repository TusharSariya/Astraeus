...
const base = process.env.BENCH_URL ?? 'http://127.0.0.1:5242'
const output = process.env.BENCH_PROOF_DIR ?? '/tmp/astraeus-runs-proof'
...
try {
  await page.goto(`${base}/?lat=47.5123456789&lon=-52.6987654321&t=${at}&view=Series&stack=${encodeURIComponent(JSON.stringify([{ id: layerId, visible: true, opacity: .85 }]))}`)
  await page.getByRole('img', { name: /temperature_2m native samples/ }).waitFor()
  const control = page.getByRole('combobox', { name: 'Browsing run for eccc-hrdps' })
  await control.selectOption('old')
  await page.getByRole('heading', { name: 'temperature_2m · eccc-hrdps · old', exact: true }).waitFor()
  assert.equal(new URL(page.url()).searchParams.get('run.eccc-hrdps'), 'old')
  await page.getByRole('button', { name: 'Temporary Compare', exact: true }).click()
  await page.getByRole('button', { name: 'Compare latest and previous runs of Series A', exact: true }).click()
  await page.getByRole('img', { name: /Same-field run overlay/ }).waitFor()
  assert.equal(new URL(page.url()).searchParams.get('run.eccc-hrdps'), 'old')
  assert.equal(await page.locator('.native-series figure circle').count(), 2)
  assert.equal(await page.locator('.native-series figure rect').count(), 2)
  for (const name of ['light', 'dark', 'Red night']) {
    await page.getByRole('button', { name, exact: true }).click()
    await page.getByRole('img', { name: /Same-field run overlay/ }).scrollIntoViewIfNeeded()
    await page.screenshot({ path: `${output}/run-compare-${name.replace(' ', '-')}.png` })
  }
  await page.evaluate(() => { document.documentElement.style.fontSize = '200%' })
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true)
  await page.screenshot({ path: `${output}/run-compare-text-zoom.png` })
  await page.evaluate(() => { document.documentElement.style.fontSize = '' })
  const readCount = seriesReads
  await page.getByRole('button', { name: 'Map', exact: true }).click()
  await page.getByText(/0 of 1 layers drawn/).waitFor()
  await page.getByRole('button', { name: /Inspect layer HRDPS total cloud/ }).click()
  await page.getByRole('heading', { name: /Evidence · HRDPS total cloud/ }).waitFor()
  assert.ok(await page.getByRole('complementary', { name: /Evidence inspector/ }).textContent().then((text) => text.includes('cannot request that named run')))
  await page.screenshot({ path: `${output}/map-pinned-refusal.png` })
  await page.keyboard.press('Escape')
  await page.getByRole('button', { name: 'Series', exact: true }).click()
  await page.getByRole('img', { name: /Same-field run overlay/ }).waitFor()
  assert.equal(seriesReads, readCount)
  await page.getByRole('button', { name: 'Stop run comparison', exact: true }).click()
  await page.getByRole('heading', { name: 'temperature_2m · eccc-hrdps · old', exact: true }).waitFor()
  await page.getByRole('button', { name: 'Map', exact: true }).click()
  await page.locator('.bench-run-pin summary').click()
  await page.getByRole('button', { name: 'Use Latest available for eccc-hrdps', exact: true }).click()
  await page.getByText(/1 of 1 layers drawn/).waitFor()
  assert.equal(new URL(page.url()).searchParams.has('run.eccc-hrdps'), false)
  assert.deepEqual(errors, [])
  await writeFile(`${output}/receipt.json`, JSON.stringify({ at, viewport: '1440x900', weatherProviderRequests: 0, externalRequests: 'blocked', checks: ['named pin in URL', 'temporary same-field overlay leaves browsing pin unchanged', 'native zero and missing sample', 'three themes', '200% text zoom without page overflow', 'Map named-run refusal with inspector', 'view switch retains finite selection', 'explicit Latest restores Map'], requests, errors }, null, 2))
  console.log(`PASS: fixed named-run desktop proof at ${output}`)
