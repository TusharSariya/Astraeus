...
try {
  ...
  if (!before) {
    ...
    await page.getByRole('searchbox',{name:'Search layers and sources'}).fill('no matches')
    await page.getByText('No matching sources or layers. Clear filters to browse all entries.').waitFor()
    await page.getByRole('searchbox',{name:'Search layers and sources'}).fill('HRDPS')
    assert.equal(await page.locator('.discovery-card').count(),1)
    ...
    await page.getByRole('button',{name:'Browse',exact:true}).click()
    await page.getByText('Capabilities · 1',{exact:true}).click(); await page.getByRole('button',{name:/^Add HRDPS/}).click()
