...
const base = process.env.BENCH_URL ?? 'http://127.0.0.1:5241'
const output = process.env.BENCH_PROOF_DIR ?? '/tmp/astraeus-layer-proof'
...
try {
  ...
  await page.getByText(/Layer frame identities/).click()
  await page.getByText(/Source mapping known/).waitFor()
  await page.getByText('fixture-layer-source · total_cloud_opacity', { exact: true }).waitFor()
  await page.getByText(/Advertised image times · 1/).click()
  await page.getByText('2026-09-07T13:00:00Z', { exact: true }).waitFor()
  assert.match(await page.getByText(/Listed native frames:/).textContent(), /12:00:00/)
  await page.locator('.sources-unmapped').screenshot({ path: `${output}/layer-image-inventory.png` })
  await page.getByRole('button', { name: 'Inspect source fixture-layer-source', exact: true }).click()
  await page.getByRole('heading', { name: 'Evidence · fixture-layer-source', exact: true }).waitFor()
  for (const theme of ['light', 'dark', 'Red night']) {
    await page.getByRole('button', { name: theme, exact: true }).click()
    await page.screenshot({ path: `${output}/layer-identity-${theme.replace(' ', '-')}.png` })
  }
  await page.getByRole('button', { name: 'Close inspector', exact: true }).click()
