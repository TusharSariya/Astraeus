...
const base = process.env.BENCH_URL ?? 'http://127.0.0.1:5249'
const output = process.env.BENCH_PROOF_DIR ?? '/tmp/astraeus-series-keyboard-proof'
...
try {
  await page.goto(`${base}/?view=Series&lat=47.5123456789&lon=-52.6987654321&t=${at}`)
  const series = page.getByRole('region',{name:'Native Series',exact:true})
  await series.locator('.native-track').first().waitFor()
  ...
  const window = series.getByRole('combobox',{name:'Window from Focus'})
  await tabTo(window,true)
  await page.keyboard.press('6')
  await page.keyboard.press('Tab')
  assert.equal(await window.inputValue(),'6')
  await inspector.getByText(/Native selection expired or changed/).waitFor()
  assert.equal(await inspector.getByText('Source',{exact:true}).count(),0)
  await tabTo(inspector.getByRole('button',{name:'Close inspector'}))
  await page.keyboard.press('Escape')
  const returnedToWindow = await window.evaluate(el=>el===document.activeElement)
  assert.equal(returnedToWindow,true)
  ...
  await page.keyboard.press('Tab')
  await page.keyboard.press('Enter')
  assert.equal(await window.evaluate(el=>el===document.activeElement),true)
  ...
  await page.screenshot({path:`${output}/window-change-return.png`})
  await writeFile(`${output}/receipt.json`,JSON.stringify({returnedToWindow,focus,oldEvidenceCleared:true,seriesReads,errors},null,2))
  console.log(JSON.stringify({returnedToWindow,focus,oldEvidenceCleared:true,seriesReads,errors},null,2))
