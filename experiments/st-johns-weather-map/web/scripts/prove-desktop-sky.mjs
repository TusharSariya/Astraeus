...
const base = process.env.BENCH_URL ?? 'http://127.0.0.1:5240'
const output = process.env.BENCH_PROOF_DIR ?? '/tmp/astraeus-sky-proof'
...
try {
  ...
  await page.getByRole('img', { name: 'Reference grid only; no registered horizon', exact: true }).waitFor()
  assert.equal(await page.locator('.sky-registered-horizon').count(), 0)
  await page.getByRole('img', { name: 'Low cloud: 0 %; scalar gauge, not direction', exact: true }).waitFor()
  await page.getByText(/Retrieval ineligible: partnership_only/).waitFor()
  assert.equal(await page.locator('.sky-instrument img').count(), 0)
  await page.locator('.bench-place summary').click()
  await page.getByRole('button', { name: 'Use Signal Hill as Focus', exact: true }).click()
  await page.locator('.sky-registered-horizon').waitFor()
  await page.locator('.bench-place summary').click()
  await page.getByText('CONSTRUCTED GEOMETRY CONTRACT FIXTURE · not an ephemeris calculation', { exact: true }).waitFor()
  assert.ok(requests.some((request) => request.includes('/astronomy?latitude=47.5704') && request.includes('longitude=-52.6816')))
  for (const name of ['light', 'dark', 'Red night']) {
    await page.getByRole('button', { name, exact: true }).click()
    await page.screenshot({ path: `${output}/sky-${name.replaceAll(' ', '-')}.png` })
  }
  failAstronomy = true
  ...
  await page.locator('.bench-instant summary').click()
  await page.getByText(/Geometry unavailable: astronomy returned 503/).waitFor()
  assert.ok(requests.some((request) => request.includes('/astronomy?') && request.includes('12%3A00%3A12.345Z')))
  assert.equal(await page.locator('.sky-registered-horizon').count(), 1)
