...
/** Weather-minute interval, used by manual steps and once per second by Play. */
export const PLAYBACK_SPEEDS = [1, 2, 4, 8, 15, 30] as const
...
/** The transport's readout: the speed with its direction, e.g. "15 min each second"
 *  or "15 min each second reversed". Callers render the arrow; this is the text an
 *  assistive reader hears. */
export function describeSpeed(speed: number, direction: PlaybackDirection): string {
  return `${speed} min each second${direction === -1 ? ' reversed' : ''}`
}
