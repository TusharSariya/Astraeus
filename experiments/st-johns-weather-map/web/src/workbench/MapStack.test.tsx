...
it('repairs a retired selection only on request, preserving its position and settings and returning focus', async () => {
  ...
  expect(screen.getByRole('checkbox', { name: 'Show HRDPS total cloud (live proxy)' })).toHaveFocus()
})
...
it.each([
  ...
  expect(screen.getByText(message, { exact: false, selector: '.bench-layer-state' })).toBeInTheDocument()
})
...
it('distinguishes a successful empty feature response from unavailable data', () => {
  ...
  expect(screen.getByText(/No features returned/, { selector: '.bench-layer-state' })).toBeInTheDocument()
  expect(screen.queryByText(/Unavailable · no frame drawn/)).not.toBeInTheDocument()
})
