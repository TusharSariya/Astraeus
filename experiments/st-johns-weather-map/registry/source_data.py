...
DATAMART_FALLBACK = {
    ...
    "eccc-gdps": f"{ECCC_DATAMART}{{YYYYMMDD}}/WXO-DD/model_gdps/15km/{{HH}}/{{FFF}}/",
}
...
def registry() -> dict[str, Any]:
    ...
    s.extend([
        ...
        _eccc_model("eccc-gdps", "deterministic_forecast", "GDPS", "nwp_gdps", ["temperature", "humidity", "wind", "pressure", "precipitation", "cloud", "surface_state"], ["surface", "2 m", "10 m", "isobaric", "column", "soil"], "2 runs/day", "10 days", "Broader Canadian deterministic context", True, reach=_reach(0, 240), run_cadence_seconds=RUN_CADENCE_2X_DAILY, publication_latency=NO_LATENCY, endpoints=[DATAMART_FALLBACK["eccc-gdps"]], datamart_fallback_path=DATAMART_FALLBACK["eccc-gdps"], reason_suffix=" The atmospheric forecast uses the dated 15 km LatLon0.15 tree; the separate 10 km tree carries sea-ice analysis NetCDF and is not a forecast fallback."),
        ...
        _source(
            "metoffice-ostia-sst", "analysis",
            "catalogued", "Isolated acquisition is authorized by the shared source-integration experiment, but no owner source-admission resolution names this new source id, so it remains catalogued and unschedulable. Anonymous current access verified through the Copernicus Marine ARCO time-chunked Zarr delivery. The product is the Met Office near-real-time Level 4 daily foundation SST analysis; its analysis error and native surface bit mask are retained. SST alone is not evidence that fog will occur.",
            "Met Office", "OSTIA Global Ocean foundation SST analysis (METOFFICE-GLO-SST-L4-NRT-OBS-SST-V2)",
            ["https://data.marine.copernicus.eu/product/SST_GLO_SST_L4_NRT_OBSERVATIONS_010_001/description", "https://documentation.marine.copernicus.eu/QUID/CMEMS-SST-QUID-010-001.pdf"],
            ...
            ("typed_adapter", "bounded anonymous HTTPS reader for consolidated timeChunked Zarr metadata and only the native chunks intersecting the evidence box"),
            ["sea_surface_temperature", "sea_surface_temperature_uncertainty", "sea_surface_temperature_mask"], ["foundation sea surface"],
            "Global 0.05 degree WGS 84 grid; stored only for 45.0 to 50.5 N, 58.0 to 46.0 W", "daily; update by 12:00 UTC the following day", "analysis instant only",
            (False, "anonymous HTTPS", None),
            {"licence": {"name": "Copernicus Marine Service data licence, CC BY 4.0", "url": "https://marine.copernicus.eu/user-corner/service-commitments-and-licence", "review_state": "verified"}, "attribution": "Credit the Met Office, GHRSST and Copernicus Marine Service; retain product id and DOI 10.48670/moi-00165.", "caching": "Only evidence-box native chunks, inside the 64 GiB hot cache and per-operation bounds.", "archival": "Rolling current-analysis evidence only; historical acquisition remains ticket 94.", "redistribution": "CC BY 4.0 with the required producer and Copernicus Marine attribution."},
            "GHRSST GDS 2.4 / CF-1.4", "48 h", "Current observed-analysis SST input for coastal conditions; no fog formula or score", (False, None, "An SST analysis is not an atmospheric consensus vote."), "passing", "passing",
            delivery_kind="published_cell", reach=OBSERVED_INSTANT, native_cadence_seconds=86400, publication_latency=_latency(129600, "Copernicus Marine target delivery is by 12:00 UTC on the following day; conservative 36 h seed"),
        ),
        _source(
            "noaa-oisst-v2-1", "analysis",
            "catalogued", "Isolated acquisition is authorized by the shared source-integration experiment, but no owner source-admission resolution names this new source id, so it remains catalogued and unschedulable. Anonymous current NCEI daily files verified. Discovery accepts the provider's preliminary suffix and preserves preliminary versus final identity; the native estimated error standard deviation is retained. SST alone is not evidence that fog will occur.",
            "NOAA National Centers for Environmental Information", "NOAA 0.25-degree Daily OISST AVHRR-only v2.1",
            ["https://www.ncei.noaa.gov/products/optimum-interpolation-sst", "https://doi.org/10.25921/RE9P-PT57"],
            ["https://www.ncei.noaa.gov/data/sea-surface-temperature-optimum-interpolation/v2.1/access/avhrr/"],
            ("typed_adapter", "bounded anonymous HTTPS directory discovery and NetCDF4 decode"),
            ["sea_surface_temperature", "sea_surface_temperature_uncertainty"], ["sea surface analysis"],
            "Global 0.25 degree grid; stored only for 45.0 to 50.5 N, 58.0 to 46.0 W", "daily", "analysis instant only",
            (False, "anonymous HTTPS", None), OPEN_US_POLICY, "CF-1.6 / OISST v2.1", "72 h",
            "Independent coarse current SST analysis input for coastal conditions; no fog formula or score", (False, None, "An SST analysis is not an atmospheric consensus vote."), "passing", "passing",
            delivery_kind="published_cell", reach=OBSERVED_INSTANT, native_cadence_seconds=86400, publication_latency=_latency(172800, "NCEI describes preliminary near-real-time production at about one-day latency; conservative 48 h seed"),
        ),
        _eccc_model(
    ...
    s.extend([
        _source("eccc-swob", "surface_observation", *_admission("eccc-swob", "Canonical GeoMet OGC API Features `swob-realtime` is queried at the exact selected report instant; only MSC surface-weather datasets are admitted by native response identity."), "Environment and Climate Change Canada", "MSC SWOB real-time surface observations via GeoMet OGC API Features", [ECCC_CATALOGUE, f"{ECCC_API}collections/swob-realtime"], [f"{ECCC_API}collections/swob-realtime/items"], ("typed_adapter", "bounded httpx GeoJSON demand query with isolated decoder"), ["air_temp", "dwpt_temp", "rel_hum", "mslp", "wnd_spd", "wnd_dir", "field_quality_tokens"], ["station surface"], "Nearest published MSC station within the accepted 0.75 latitude-corrected-degree ceiling", "exact native report instant; station-dependent", "observations only", (False, "Anonymous HTTPS", None), ECCC_POLICY, "GeoMet OGC API Features `swob-realtime` response schema", "finite source-local receipt cache only; expired values are withheld", "Official MSC station observation evidence; not a forecast consensus input", (False, None, "Observations do not vote in forecast consensus."), delivery_kind="published_cell", reach=OBSERVED_INSTANT, native_cadence_seconds=3600),
    ...
    s.extend([
        ...
        _source(
            ...
            "Internal experimental WeatherNext 3 surface point delivery: 21 fields with six provider statistics each. Historical and local forecast scopes use bounded authenticated native reads and generation-pinned run identities. No scheduled ingestion or primary admission.",
            ...
            ["21 surface fields with mean, p10, p25, p50, p75 and p90"], ["native surface levels"], "Native 0.1 degree and station-trained 0.05 degree cells", "Hourly provider leads; published main-cycle roots", "Up to 360 hours from a published main cycle",
            (True, "Existing approved Google gcloud profile; no WeatherNext API key or ADC mutation", "https://developers.google.com/weathernext/guides/access-forecast"),
            {"licence": {"name": "Internal experimental forecast terms; historical reader retains its conservative greater-than-48-hour age boundary", "url": "https://developers.google.com/weathernext/guides/disclaimers", "review_state": "restricted"},
             "attribution": "WeatherNext data provided by Google",
             "caching": "Bounded internal point cache; fixed 60-second receipt expiry.",
             "archival": "No current or future output archival; raw payloads are ephemeral.",
             "redistribution": "Only the authorized historical experimental path; no current/future serving permission inferred."},
            "weathernext_3_0_0_statistics", "Historical evidence; no current freshness promise",
            "Non-primary internal surface statistic comparison", (False, "google-weathernext-3", "Historical provider statistics are excluded from forecast consensus."),
            "passing", "passing", delivery_kind="published_cell", display_primary=False,
        ),
    ...
    s.extend([
        _source(
            ...
            *_admission("noaa-goes-east", "GOES-19 public S3 experimental acquisition covers ABI-L2 ACMF/ACHAF plus CCLF, ACTPF, ACHTF, CODF/COD2KMF, CPSF, TPWF, LVMPF/LVTPF, DSIF, SSTF, DMWF/DMWVF and RRQPEF. No fog or cloud-base product is claimed; every native DQF, mask, time and geometry remains explicit."),
            "NOAA/NESDIS", "GOES-19 ABI L2+ retrieval products",
            ["https://www.goes-r.gov/downloads/resources/documents/Beginners_Guide_to_GOES-R_Series_Data.pdf", "https://www.goes-r.gov/products/overview.html"],
            ["https://noaa-goes19.s3.amazonaws.com/"],
            ("typed_adapter", "anonymous HTTPS S3 listing and NetCDF reader"),
            ["cloud_mask_ABI_L2_ACM", "cloud_top_height_ABI_L2_ACHA_2km", "CCLF", "ACTPF", "ACHTF", "CODF", "COD2KMF", "CPSF", "TPWF", "LVMPF", "LVTPF", "DSIF", "SSTF", "DMWF", "DMWVF", "RRQPEF", "native quality flags and geometry"],
            ["native fixed-grid pixels", "101 producer pressure levels", "sparse motion vectors"],
            "GOES-East (GOES-19) full disk; Newfoundland is high-zenith-angle edge coverage requiring masks",
            "10 minutes most ABI full-disk products; hourly CCLF and SSTF",
            "observations and producer retrievals only", (False, "Anonymous public S3", None),
            OPEN_US_POLICY, "GOES-R product version and source filename embedded in each NetCDF",
            "30 minutes, product-dependent", "Satellite retrieval evidence with native DQF and coverage; non-operational experimental integration",
            (False, None, "Satellite observations and producer retrievals are not forecast-centre votes."),
            delivery_kind="published_cell", reach=OBSERVED_INSTANT, native_cadence_seconds=600,
        ),
        _source(
            "noaa-goes-glm", "lightning",
            *_admission("noaa-goes-glm", "GOES-19 GLM-L2-LCFA is anonymously retrievable from NOAA's public S3 bucket. Experimental acquisition retains the event/group/flash hierarchy and publishes an empty in-box collection only after a file whose declared field of view contains the evidence box was read."),
            "NOAA/NESDIS", "GOES-19 GLM L2 Lightning Cluster-Filter Algorithm",
            ["https://www.goes-r.gov/spacesegment/glm.html"], ["https://noaa-goes19.s3.amazonaws.com/"],
            ("typed_adapter", "anonymous HTTPS S3 listing and NetCDF reader"),
            ["event/group/flash latitude", "longitude", "time", "energy", "area", "parent identifiers", "quality flags"],
            ["event", "group", "flash"], "GOES-19 full-disk field of view including the evidence box",
            "20 seconds", "observations only", (False, "Anonymous public S3", None), OPEN_US_POLICY,
            "GOES-R product version and source filename embedded in each NetCDF", "30 seconds",
            "Lightning detection features; absence is evidence only after a complete observed interval",
            (False, None, "Lightning observations are not forecast-centre votes."),
            fixture="passing", live="planned", delivery_kind="published_cell",
            reach=OBSERVED_INSTANT, native_cadence_seconds=20,
        ),
    ...
    s.extend([
        ...
        _source(
            "noaa-swpc-rtsw", "space_weather", "implemented-unverified",
            "The v2 adapter stores the feed's spacecraft identity, active indicator and every quality flag per row. Live retrieval and artifact readback are receipted; production verification remains owner-controlled.",
            "NOAA Space Weather Prediction Center", "Real-time solar wind magnetic field (1-minute)",
            swpc_docs,
            ["https://services.swpc.noaa.gov/json/rtsw/rtsw_mag_1m.json"],
            ("typed_adapter", "httpx JSON via PoliteClient; valid_time x spacecraft with per-row quality flags"),
            ...
            admission_condition={"condition": "The real-time solar wind feed interleaves SWFO-L1, ACE and IMAP since DSCOVR left it, and the current adapter does not store every quality flag.", "satisfied_by": "swpc-rtsw-v2 stores the spacecraft axis and every feed quality flag; fixture, live capture and artifact readback are recorded.", "satisfied": True, "recorded_on": "2026-09-05"},
        ),
        _source(
    ...
    gfz_policy = {
        ...
        "attribution": "Credit GFZ German Research Centre for Geosciences and the named Kp/Hp index.",
    ...
    s.extend([
        _source(
            ...
            swpc_docs, ["https://services.swpc.noaa.gov/json/rtsw/rtsw_wind_1m.json"],
            ("typed_adapter", "httpx JSON via PoliteClient; valid_time x spacecraft with per-row quality flags"),
            ...
            (False, None, "Solar wind measurements are not forecast votes."), delivery_kind="published_cell", reach=OBSERVED_INSTANT, native_cadence_seconds=60,
        ),
        _source(
            ...
            ("typed_adapter", "bounded httpx JSON adapter; provider propagation instant retained"),
        ...
        _source(
            ...
            ("typed_adapter", "bounded httpx JSON adapter; provider Kp thirds code retained"),
            ...
            (False, None, "A planetary index is not a forecast-centre vote."), delivery_kind="published_cell", reach=OBSERVED_INSTANT, native_cadence_seconds=60,
        ),
        _source(
            ...
            ("typed_adapter", "bounded httpx JSON adapter; issued message text retained verbatim"),
            ["space_weather_alert"], ["alert message (no vertical level)"],
            "Planetary; no spatial coordinates are stored or claimed",
            ...
            (False, None, "Text alerts are not forecast votes."), delivery_kind="published_cell", reach=OBSERVED_INSTANT, native_cadence_seconds=1800,
        ),
        _source(
            ...
            ("typed_adapter", "bounded httpx JSON adapter; current and forecast day offsets retained"),
            ["radio_blackout_scale", "solar_radiation_storm_scale", "geomagnetic_storm_scale"], ["planetary index (no vertical level)"],
            "Planetary; no spatial coordinates are stored or claimed",
            ...
            (False, None, "Scale summaries are not forecast votes."), delivery_kind="published_cell", reach=OBSERVED_INSTANT, native_cadence_seconds=21600,
        ),
        _source(
            "gfz-kp-current", "space_weather", "catalogued",
            "Current GFZ Kp is public and live under CC BY 4.0. It remains catalogued because the 2026-09-02 owner admissions named GFZ Hp30, not this distinct product; the experimental reader is not scheduler-registered.",
            "GFZ German Research Centre for Geosciences", "Kp three-hour planetary geomagnetic index (current selection)",
            ["https://kp.gfz.de/en/data"], ["https://kp.gfz.de/app/json/?index=Kp"],
            ("typed_adapter", "bounded experimental httpx JSON reader; unregistered and never scheduled"),
            ["kp_index", "kp_status"], ["planetary index (no vertical level)"],
            "Planetary; no spatial coordinates are stored or claimed",
            "3 hours", "current observed/provisional selection only; no historical archive and no lead hours",
            (False, "Anonymous HTTPS", None), gfz_policy,
            "GFZ current Kp JSON schema as served", "6 hours",
            "Planetary geomagnetic context; the producer status token remains attached to each value",
            (False, None, "A planetary index is not a forecast-centre vote."),
            fixture="passing", live="passing", delivery_kind="published_cell",
        ),
        _source(
            ...
            ("typed_adapter", "bounded httpx JSON adapter; 24-hour selection and response licence checked"),
            ...
            "Higher-cadence geomagnetic activity evidence beside the 3-hour Kp series",
            (False, None, "A planetary index is not a forecast-centre vote."), delivery_kind="published_cell", reach=OBSERVED_INSTANT, native_cadence_seconds=1800,
        ),
        _source(
            "gfz-hp60-current", "space_weather", "catalogued",
            "Current GFZ Hp60 is public and live under CC BY 4.0. It remains catalogued because the 2026-09-02 owner admissions named GFZ Hp30, not this distinct product; the experimental reader is not scheduler-registered.",
            "GFZ German Research Centre for Geosciences", "Hp60 hourly geomagnetic index (current selection)",
            ["https://kp.gfz.de/en/hp30-hp60"], ["https://kp.gfz.de/app/json/?index=Hp60"],
            ("typed_adapter", "bounded experimental httpx JSON reader; unregistered and never scheduled"),
            ["hp60_index"], ["planetary index (no vertical level)"],
            "Planetary; no spatial coordinates are stored or claimed",
            "1 hour", "current observed selection only; no historical archive and no lead hours",
            (False, "Anonymous HTTPS", None), gfz_policy,
            "GFZ current Hp60 JSON schema as served", "2 hours",
            "Hourly planetary geomagnetic context beside Hp30; not a resampling of Hp30",
            (False, None, "A planetary index is not a forecast-centre vote."),
            fixture="passing", live="passing", delivery_kind="published_cell",
        ),
        _source(
            ...
            ("typed_adapter", "bounded httpx JSON adapter; valid_time x satellite"),
            ["geomagnetic_field_eastward", "geomagnetic_field_parallel", "geomagnetic_field_normal", "geomagnetic_field_total"], ["GOES geosynchronous orbit (no vertical level)"],
            ...
            (False, None, "Satellite magnetometer measurements are not forecast votes."), delivery_kind="published_cell", reach=OBSERVED_INSTANT, native_cadence_seconds=60,
        ),
        _source(
            ...
            ("typed_adapter", "bounded httpx JSON adapter; both channels on valid_time x satellite"),
            ...
            (False, None, "Satellite X-ray flux measurements are not forecast votes."), delivery_kind="published_cell", reach=OBSERVED_INSTANT, native_cadence_seconds=60,
        ),
        _source(
            ...
            *_admission(
            ...
            ("typed_adapter", "bounded httpx JSON adapter; Kyoto producer and SWPC intermediary retained"),
    ...
    s.extend([
        ...
        _source(
            ...
            *_admission(
            ...
            ("typed_adapter", "httpx selected-hour Marine API adapter sending model=ncep_gfswave016, cell_selection=sea, start_hour=end_hour and the nine declared native fields; no producer-run endpoint is queried"),
            ...
            dict(
                ...
                caching="Cache only a canonical selected-hour point response in the source-local bounded in-memory cache for at most five minutes, shortened by a provider freshness directive when present; no meta.json or producer-run stamp is fetched.",
                archival="No provider response or producer-run artifact is retained; an expired entry is discarded and never served as a fallback.",
            ),
            "Open-Meteo Marine JSON for the selected hourly point as served; the response does not expose a producer-run identifier",
            "retrieval time captured immediately after the final response byte; finite source-local cache freshness only, shortened by a provider freshness directive when present",
