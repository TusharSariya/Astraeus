...
FIELDS: list[dict[str, Any]] = [
    ...
    *[
    ...
    _f("space_weather_alert", "space weather alert text", "text", "space_weather",
       "planetary", "space_weather_guidance", "An issued NOAA alert, watch or warning retained verbatim."),
    _f("radio_blackout_scale", "NOAA radio blackout scale", "1", "space_weather",
       "planetary", "space_weather_guidance", "NOAA R scale by the product's own day offset.", value_range=(0.0, 5.0)),
    _f("solar_radiation_storm_scale", "NOAA solar radiation storm scale", "1", "space_weather",
       "planetary", "space_weather_guidance", "NOAA S scale by the product's own day offset.", value_range=(0.0, 5.0)),
    _f("geomagnetic_storm_scale", "NOAA geomagnetic storm scale", "1", "space_weather",
       "planetary", "space_weather_guidance", "NOAA G scale by the product's own day offset.", value_range=(0.0, 5.0)),
    _f("geomagnetic_field_eastward", "geosynchronous magnetic field eastward component", "nT", "space_weather",
       "geosynchronous orbit", "magnetic_field", "GOES magnetometer He component, stored per satellite."),
    _f("geomagnetic_field_parallel", "geosynchronous magnetic field spin-axis component", "nT", "space_weather",
       "geosynchronous orbit", "magnetic_field", "GOES magnetometer Hp component, stored per satellite."),
    _f("geomagnetic_field_normal", "geosynchronous magnetic field normal component", "nT", "space_weather",
       "geosynchronous orbit", "magnetic_field", "GOES magnetometer Hn component, stored per satellite."),
    _f("geomagnetic_field_total", "geosynchronous magnetic field magnitude", "nT", "space_weather",
       "geosynchronous orbit", "magnetic_field", "GOES magnetometer total field, stored per satellite."),
    ...
    _f("sea_surface_temperature_uncertainty", "sea surface temperature analysis uncertainty", "degC", "marine", "sea surface",
       ...
       standard_name="sea_surface_temperature standard_error", value_range=(0.0, 100.0)),
    _f("sea_surface_temperature_mask", "sea surface temperature native surface mask", "flag", "marine", "sea surface",
       "sea_surface_temperature_mask",
       "The producer's native water, land, lake and sea-ice bit mask retained for quality and missingness. It is not served as sea-ice evidence."),
    ...
    _f("wind_wave_period", "wind wave period", "s", "marine", "sea surface", "wave_partition_period",
       "Period of the wind-sea partition, as the producer defines it."),
    _f("swell_wave_period", "swell wave period", "s", "marine", "sea surface", "wave_partition_period",
       "Period of the swell partition, as the producer defines it."),
    _f("wind_wave_direction", "wind wave direction", "degree", "marine", "sea surface", "wave_partition_direction",
       "From-direction of the wind-sea partition.", value_range=(0.0, 360.0)),
    _f("swell_wave_direction", "swell wave direction", "degree", "marine", "sea surface", "wave_partition_direction",
       "From-direction of the swell partition.", value_range=(0.0, 360.0)),
...
SOURCE_FIELDS: list[dict[str, Any]] = [
    ...
    *[_sf("noaa-goes-east", f"cloud_fraction_layer_{index}", f"ABI-L2-CCLF CF{index}", "stored",
    ...
    _sf("noaa-swpc-plasma", "solar_wind_density", "rtsw_wind_1m.json proton_density", "available-not-stored", "Demand-queried from one native spacecraft row with every feed quality flag; no artifact fallback."),
    _sf("noaa-swpc-plasma", "solar_wind_speed", "rtsw_wind_1m.json proton_speed", "available-not-stored", "Demand-queried from the same native row as density; no artifact fallback."),
    _sf("noaa-swpc-plasma", "solar_wind_temperature", "rtsw_wind_1m.json proton_temperature", "available-not-stored", "Demand-queried from the same native row as density; no artifact fallback."),
    ...
    _sf("noaa-swpc-propagated-solar-wind", "bz_gsm", "propagated-solar-wind-1-hour.json bz", "stored", "Provider-propagated value, disclosed as such."),
    _sf("noaa-swpc-propagated-solar-wind", "bt", "propagated-solar-wind-1-hour.json bt", "stored", "Provider-propagated value, disclosed as such."),
    _sf("noaa-swpc-kp-1m", "kp_index", "planetary_k_index_1m.json kp_index", "stored", "One-minute planetary index with the provider's thirds code retained."),
    _sf("noaa-swpc-alerts", "space_weather_alert", "alerts.json message", "stored", "Issued text retained verbatim, including line endings."),
    _sf("noaa-swpc-scales", "radio_blackout_scale", "noaa-scales.json R.Scale", "stored", "Stored by provider day offset."),
    _sf("noaa-swpc-scales", "solar_radiation_storm_scale", "noaa-scales.json S.Scale", "stored", "Stored by provider day offset."),
    _sf("noaa-swpc-scales", "geomagnetic_storm_scale", "noaa-scales.json G.Scale", "stored", "Stored by provider day offset."),
    _sf("gfz-hp30", "hp30_index", "GFZ JSON Hp30", "stored", "Bounded 24-hour selection; response licence checked."),
    _sf("gfz-kp-current", "kp_index", "GFZ JSON Kp", "available-not-stored", "Bounded current selection verified experimentally; source remains catalogued."),
    _sf("gfz-kp-current", "kp_status", "GFZ JSON status", "available-not-stored", "Per-value producer token retained verbatim by the experimental reader."),
    _sf("gfz-hp60-current", "hp60_index", "GFZ JSON Hp60", "available-not-stored", "Bounded current selection verified experimentally; source remains catalogued."),
    _sf("noaa-goes-magnetometer", "geomagnetic_field_eastward", "magnetometers-1-day.json He", "stored", "Stored per GOES satellite."),
    _sf("noaa-goes-magnetometer", "geomagnetic_field_parallel", "magnetometers-1-day.json Hp", "stored", "Stored per GOES satellite."),
    _sf("noaa-goes-magnetometer", "geomagnetic_field_normal", "magnetometers-1-day.json Hn", "stored", "Stored per GOES satellite."),
    _sf("noaa-goes-magnetometer", "geomagnetic_field_total", "magnetometers-1-day.json total", "stored", "Stored per GOES satellite."),
    _sf("noaa-goes-xray", "xray_flux_short", "xrays-1-day.json flux at 0.05-0.4nm", "stored", "Stored per GOES satellite and channel."),
    _sf("noaa-goes-xray", "xray_flux_long", "xrays-1-day.json flux at 0.1-0.8nm", "stored", "Stored per GOES satellite and channel."),
    _sf("noaa-swpc-kyoto-dst", "dst_index", "kyoto-dst.json dst", "stored", "Kyoto WDC value relayed by SWPC; reprocessed and never display primary."),
    ...
    _sf("eccc-swob", "temperature_2m", "air_temp", "available-not-stored",
        "Exact-time MSC SWOB station observation from the bounded GeoMet demand query."),
    _sf("eccc-swob", "dew_point_2m", "dwpt_temp", "available-not-stored",
        "Exact-time MSC SWOB station observation; absent or failed-QC values remain null."),
    _sf("eccc-swob", "relative_humidity_2m", "rel_hum", "available-not-stored",
        "Native relative-humidity value only; no humidity derivation or cross-fill."),
    _sf("eccc-swob", "mean_sea_level_pressure", "mslp", "available-not-stored",
        "Native mean-sea-level pressure from the selected station report."),
    _sf("eccc-swob", "wind_speed_10m", "wnd_spd", "available-not-stored",
        "Native station wind speed, retained separately from wind direction."),
    _sf("eccc-swob", "wind_direction_10m", "wnd_dir", "available-not-stored",
        "Native station wind direction; no u/v reconstruction or direction conversion."),
    ...
    _sf("metoffice-ostia-sst", "sea_surface_temperature", "analysed_sst", "stored",
        "Daily 0.05 degree foundation SST; Kelvin is normalized to degC while original units remain in provenance."),
    _sf("metoffice-ostia-sst", "sea_surface_temperature_uncertainty", "analysis_error", "stored",
        "Producer-published analysis standard error."),
    _sf("metoffice-ostia-sst", "sea_surface_temperature_mask", "mask", "stored",
        "Native bit mask retained; land is masked from SST and the mask is not treated as sea-ice evidence."),
    _sf("noaa-oisst-v2-1", "sea_surface_temperature", "sst", "stored",
        "Daily 0.25 degree AVHRR-only OISST analysis; preliminary identity is retained from the file id."),
    _sf("noaa-oisst-v2-1", "sea_surface_temperature_uncertainty", "err", "stored",
        "Producer-published estimated error standard deviation."),
