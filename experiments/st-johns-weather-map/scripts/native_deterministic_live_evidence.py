...
def retained_candidate(adapter: object, source_dir: Path) -> RunCandidate | None:
    ...
    if provenance_path.is_file():
        original_retrieved_at = datetime.fromisoformat(json.loads(provenance_path.read_text())["retrieved_at"])
    if isinstance(adapter, IndexedNativeCandidate):
        ...
        retained_bytes = discovery.get("raw_bytes")
        retained_digest = discovery.get("raw_sha256")
        if not isinstance(retained_bytes, int) or not isinstance(retained_digest, str):
            return None
        return RunCandidate(discovery["provider_run_id"], run, discovery["urls"], {"index": index, "retained_raw": True,
            "retained_raw_bytes": retained_bytes, "retained_raw_sha256": retained_digest, "retained_retrieved_at": original_retrieved_at})
    ...
    return RunCandidate(discovery["provider_run_id"], run, discovery["urls"], {"cycle": run.strftime("%H"), "stamp": discovery["provider_run_id"], "retained_raw": True, "retained_retrieved_at": original_retrieved_at})


def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("output", type=Path)
    parser.add_argument("--offline", type=Path, metavar="RETAINED_INPUT",
                        help="replay a retained bundle into output without provider requests")
    args = parser.parse_args()
    output = args.output.resolve()
    retained_input = args.offline.resolve() if args.offline else None
    if retained_input is not None and (retained_input == output or retained_input in output.parents or output in retained_input.parents):
        parser.error("offline replay input and output must not overlap")
    ...
    summary: dict[str, object] = {"replayed_at" if retained_input else "retrieved_at": datetime.now(UTC).isoformat(), "bounds": [45.0, -58.0, 50.5, -46.0],
    ...
    for adapter in adapters:
        ...
        candidate = None
        try:
            retained_source = retained_input / adapter.source_id if retained_input is not None else source_dir
            candidate = retained_candidate(adapter, retained_source)
            ...
            elif retained_input is not None:
                shutil.copytree(retained_source, source_dir, dirs_exist_ok=True)
            result = adapter.fetch(candidate, window, source_dir)
            artifact = result.artifacts[0]; provenance_path = source_dir / "provenance.json"
            if retained_input is not None:
                retained_artifact = retained_source / artifact.payload_path.name
                retained_provenance_path = retained_source / "provenance.json"
                if not retained_artifact.is_file() or not retained_provenance_path.is_file():
                    raise AssertionError(f"retained artifact evidence is incomplete: {result.source_id}")
                retained_provenance = json.loads(retained_provenance_path.read_text())
                expected_artifact_digest = retained_provenance.get("sha256")
                if expected_artifact_digest != sha256(retained_artifact):
                    raise AssertionError(f"retained artifact digest mismatch: {result.source_id}")
                if artifact.byte_size != retained_artifact.stat().st_size or sha256(artifact.payload_path) != expected_artifact_digest:
                    raise AssertionError(f"replayed artifact differs from retained artifact: {result.source_id}")
            if isinstance(adapter, IndexedNativeCandidate) and retained_input is None:
                discovery_path = source_dir / "discovery.json"
                discovery = json.loads(discovery_path.read_text())
                discovery.update(raw_bytes=artifact.provenance["raw_bytes"], raw_sha256=artifact.provenance["raw_sha256"])
            ...
            if retained_input is not None and not str(error).startswith("regional_exclusion:"):
                raise
            raw = source_dir / f"{adapter.source_id}.grib2"
            if retained_input is None and isinstance(adapter, IndexedNativeCandidate) and raw.is_file():
                discovery_path = source_dir / "discovery.json"
                discovery = json.loads(discovery_path.read_text())
                discovery.update(raw_bytes=raw.stat().st_size, raw_sha256=sha256(raw))
                discovery_path.write_text(json.dumps(discovery, indent=2, sort_keys=True) + "\n")
