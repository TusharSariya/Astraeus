"""Historical WeatherNext temperature mean point delivery, isolated experiment.
...
def point_evidence(payload, selection, configuration, *, completed_at, data_mode='live'):
    ...
    flags=['historical_forecast','provider_qc_not_supplied']+(['native_fill_mask'] if value is None else [])
    return EvidenceField(field=FIELD,key=FIELD,value=None if value is None else value-273.15,storage='available-not-stored',
        provenance=Provenance(data_mode=data_mode,evidence_class='retrieved',source_id=SOURCE_ID,provider='Google',
            ...
            licence='WeatherNext historical data terms; conservative valid-time age greater than 48 hours',
            attribution='WeatherNext data provided by Google',delivery_kind='published_cell',source_display_primary=False,
            adapter_version='weathernext-historical-temperature-v1',source_acquisition=acquisition,
            sampled_latitude=lat,sampled_longitude=lon,sample_method='rectilinear',run_stale=True,
            run_stale_reason='Explicit historical run; not a current forecast',
...
class WeatherNextHistoricalDelivery:
    ...
    def read_point(self,latitude,longitude,selected,*,run='latest',refresh=False):
        if run not in ('latest',self.config.run_id): raise WeatherNextDeliveryUnavailable('WeatherNext configured historical run only')
        try:
            selection=WeatherNextSelection(self.config.initialization,selected,latitude,longitude,(NATIVE_FIELD,))
            if selection.valid_time>=self._utcnow()-HISTORICAL_DELAY: raise ValueError('historical gate')
        except Exception:
            raise WeatherNextDeliveryUnavailable('WeatherNext historical selection unavailable') from None
        with self._lock:
        ...
        try:
            ...
            if selection.valid_time>=completed-HISTORICAL_DELAY: raise ValueError('historical gate')
            evidence=point_evidence(payload,selection,self.config,completed_at=completed,data_mode=self._data_mode)
