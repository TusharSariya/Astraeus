...
def _exception_code(body: bytes) -> str:
    ...
    for element in root.iter():
        if element.tag.rsplit("}", 1)[-1] == "ServiceException":
            return (element.get("code") or "unknown")[:128]
    return "unknown"
...
def normalize_capabilities(body: bytes) -> dict[str, object]:
    if b"ServiceException" in body:
        raise ValueError("lightning capabilities contains OGC service exception: " + _exception_code(body))
    root = ElementTree.fromstring(body)
    layer = None
...
def normalize_sample(body: bytes) -> dict[str, object]:
    if b"ServiceException" in body:
        raise ValueError("lightning sample contains OGC service exception: " + _exception_code(body))
    ...
    if not features:
        raise ValueError("empty feature list does not establish the accepted bare-object no-detection response")
    if document.get("type") != "FeatureCollection" or document.get("layer") != LAYER:
        raise ValueError("lightning response has unexpected source layer identity")
    feature = features[0]
    ...
    if not -90 <= latitude <= 90 or not -180 <= longitude <= 180:
        raise ValueError("lightning coordinate is outside geographic bounds")
    raw_value = properties.get("value")
    if isinstance(raw_value, bool):
        raise ValueError("boolean is not a lightning density")
    try:
        value = float(raw_value)
    except (TypeError, ValueError) as error:
        raise ValueError("lightning density is not numeric") from error
    if not math.isfinite(value) or value < 0:
        raise ValueError("lightning density is not finite and nonnegative")
