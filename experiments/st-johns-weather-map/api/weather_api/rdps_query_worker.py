"""One bounded RDPS selected-frame child; no retained publication."""
...
from ingest.adapters.eccc_datamart import ECCCDataMartAdapter, RDPS_DEMAND_VARS
...
def main() -> None:
    ...
    adapter = ECCCDataMartAdapter(source_id="eccc-rdps", model_subpath="model_rdps/10km",
                                  grid_token="RLatLon0.09", var_map=RDPS_DEMAND_VARS,
                                  bounds=request["bounds"], base_url=request["base_url"], adapter_version="rdps-demand-v1")
    ...
    with tempfile.TemporaryDirectory(prefix="rdps-child-", dir=output.parent) as directory:
        ...
        if not result.complete or not result.qc_passed:
            raise ValueError("RDPS selected native fields failed completeness/QC")
        if set(Path(directory).iterdir()) != {artifact.payload_path}:
            raise ValueError("RDPS decode left unexpected workspace files")
