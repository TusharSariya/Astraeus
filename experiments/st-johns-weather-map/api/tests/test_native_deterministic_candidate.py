...
def test_indexed_offline_replay_refuses_same_size_digest_mismatch(tmp_path: Path) -> None:
    adapter = IndexedNativeCandidate("ecmwf-ifs-native")
    row = _ecmwf("2t")
    index = (json.dumps(row) + "\n").encode()
    raw = b"GRIB7777x"
    (tmp_path / "ecmwf-ifs-native.grib2").write_bytes(raw)
    candidate = RunCandidate("2026090506", datetime(2026, 9, 5, 6, tzinfo=timezone.utc), ["data", "index"],
        {"index": index, "retained_raw": True, "retained_raw_bytes": len(raw), "retained_raw_sha256": "0" * 64})
    with pytest.raises(AdapterUnavailable, match="retained raw bundle digest mismatch"):
        adapter.fetch(candidate, FetchWindow(datetime.now(timezone.utc), 0, 0), tmp_path)
...
def test_indexed_replay_preserves_original_retrieval_time(monkeypatch, tmp_path: Path) -> None:
    import hashlib
    import xarray
    from ingest.experimental import native_deterministic as module

    raw = b"GRIB7777x"
    original = datetime(2026, 9, 5, 7, 13, tzinfo=timezone.utc)
    row = _ecmwf("2t")
    (tmp_path / "ecmwf-ifs-native.grib2").write_bytes(raw)
    lat, lon = numpy.meshgrid(numpy.linspace(44, 51, 20), numpy.linspace(-59, -45, 30), indexing="ij")
    _, geometry = coverage(lat, lon)
    monkeypatch.setattr(module, "_decode_messages", lambda *_args: (
        xarray.Dataset({"temperature_2m": (("latitude", "longitude"), [[12.0]])},
                       coords={"latitude": [47.5], "longitude": [-52.5]}), geometry, []))
    detail = {"index": (json.dumps(row) + "\n").encode(), "retained_raw": True,
              "retained_raw_bytes": len(raw), "retained_raw_sha256": hashlib.sha256(raw).hexdigest(),
              "retained_retrieved_at": original}
    candidate = RunCandidate("2026090506", datetime(2026, 9, 5, 6, tzinfo=timezone.utc), ["data", "index"], detail)
    adapter = IndexedNativeCandidate("ecmwf-ifs-native")
    result = adapter.fetch(candidate, FetchWindow(original, 0, 0), tmp_path)
    assert result.retrieved_at == original
    assert result.artifacts[0].provenance["retrieved_at"] == original.isoformat()
    detail.pop("retained_retrieved_at")
    with pytest.raises(AdapterUnavailable, match="original retrieval time"):
        adapter.fetch(candidate, FetchWindow(original, 0, 0), tmp_path)
