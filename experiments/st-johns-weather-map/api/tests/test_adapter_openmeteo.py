...
def test_retained_profile_http_comparator_rejects_changed_values_or_masks(pressure, replacement):
    ...
    # Hand-trimmed from the 2026-09-05 live capture (receipts in
    # docs/research/wayfinder/openmeteo-pressure-profiles.md): the first valid
    # hour and the canonical temperature/wind arrays at all 16 JMA levels.
    bundle = Path(__file__).resolve().parent / "fixtures/openmeteo_profiles"
    source_id = "openmeteo-jma-gsm"
    provider = json.loads((bundle / f"{source_id}.response.trimmed.json").read_text())
    response = json.loads((bundle / f"{source_id}.profile-api.trimmed.json").read_text())
