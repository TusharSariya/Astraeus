...
def test_identity_and_cadence_refuse_before_loader():
 calls=[]; s=GEFSQueryService(lambda k:calls.append(k) or entry(k),preflight=demand_operation_bounds)
...
def test_failure_is_cached_until_backoff_deadline():
 ...
 service=GEFSQueryService(fail,preflight=demand_operation_bounds,clock=lambda:now[0])
 for _ in range(2):
...
def test_noncanonical_request_refuses_before_loader(bad):
 calls=[]; service=GEFSQueryService(lambda k:calls.append(k) or entry(k),preflight=demand_operation_bounds)
...
def test_entry_rejects_member_and_absence_metadata_inconsistency():
 ...
 with pytest.raises(ValueError,match="mandatory failures"):
  GEFSQueryService(lambda k:entry(k,present=k.members[:-1]),preflight=demand_operation_bounds).query(key())
 with pytest.raises(ValueError,match="optional absences"):
  GEFSQueryService(lambda k:entry(k,optional={missing:("temperature_2m",)}),preflight=demand_operation_bounds).query(key())
