...
# Hand-trimmed samples cut from the live captures. The full-grid replay payloads
# are deliberately absent from Git (owner decision of 2026-09-05, issue #70): the
# receipts in docs/research/weathernext3-integration-evidence.md are the record of
# the live captures, and scripts/weathernext_all_fields_sample.py re-runs them
# against Google credentials.
EVIDENCE = FIXTURES / "avalon-box-six-field-4x4.json"
ALL_FIELDS_EVIDENCE = FIXTURES / "all-fields-point-lead6.json"
# The declared-box proof needs every native cell over 45-50.5N/58-46W on both
# grids, which is a whole provider response and cannot be hand-trimmed without
# weakening validate_acquisition. It is re-created by a credentialed live capture.
ALL_FIELDS_BOX_EVIDENCE = FIXTURES / "all-fields-lead6-avalon-box.json"
requires_full_box_capture = pytest.mark.skipif(
    not ALL_FIELDS_BOX_EVIDENCE.exists(),
    reason="full-coverage Avalon box capture is not stored in Git (issue #70); re-create it with "
           "scripts/weathernext_all_fields_sample.py using Google credentials",
)
...
def test_real_upstream_evidence_has_immutable_object_identities_and_finite_values():
    ...
    assert len(box["latitudes"]) == len(box["longitudes"]) == 4
...
def test_six_field_box_reaches_real_reader_and_http_with_every_selected_value(tmp_path, monkeypatch):
    ...
    expected = {item["field"]: item["leads"][0]["values"][2][1]
