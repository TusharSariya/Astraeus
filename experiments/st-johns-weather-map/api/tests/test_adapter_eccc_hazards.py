...
def test_hurricane_seasonal_empty_is_success_for_all_four_collections(tmp_path):
    ...
    assert len(result.artifacts) == 4 and result.complete
...
def test_stored_thunderstorm_artifact_reads_back_through_http(tmp_path, monkeypatch):
    import importlib
    api_module = importlib.import_module("weather_api.app")

    feature = {"type": "Feature", "id": "atlantic-v1", "geometry": {"type": "Polygon", "coordinates": []},
               "properties": {"datetime": NOW.isoformat(), "category": "MRGL"}}
    adapter = ECCCThunderstormOutlookAdapter(client({"thunderstorm_outlook": collection([feature])}), base_url="https://fixture.invalid")
    artifact = adapter.fetch(adapter.discover(WINDOW)[0], WINDOW, tmp_path / "fetch").artifacts[0]
    ...
    record = SimpleNamespace(
        ...
        provenance=artifact.provenance, run_time=NOW, retrieved_at=NOW,
        provider_run_id="outlook-proof", native_crs="OGC:CRS84",
    )
    class ArtifactStore:
    ...
    assert body["features"][0]["properties"]["category"] == "MRGL"
    assert body["features"][0]["properties"]["source_id"] == "eccc-thunderstorm-outlooks"
