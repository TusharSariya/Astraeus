...
RECEIPT_FIXTURE = (
    ...
    / "fixtures/eccc_geomet_wcs/geomet-wcs-2026-09-05.sample.json"
)
...
@pytest.mark.parametrize(("model", "field"), REPRESENTATIVE_FIELDS)
def test_live_receipt_for_every_representative_is_complete_and_self_consistent(model, field):
    """The retained live proof is the receipt, not the payload (issue #70, 2026-09-05)."""
    receipts = json.loads(RECEIPT_FIXTURE.read_text())
    record = next(
        row for row in receipts["representative_artifacts"]
        if row["coverage_id"] == field.coverage_id
    )
    acquisition = next(
        row for row in receipts["selected_acquisition"]["results"]
        if row["coverage_id"] == field.coverage_id
    )
    assert "retained_artifact" not in record and "retained_artifact" not in acquisition
    assert receipts["endpoint"] == "https://geo.weather.gc.ca/geomet/"
    assert receipts["request_parameters"]["FORMAT"] == "image/tiff"
    assert receipts["selected_acquisition"]["bounds"] == AVALON_CORE_BOUNDS

    for key in ("sha256", "run_time", "valid_time", "bytes", "status"):
        assert record[key], key
    assert re.fullmatch(r"[0-9a-f]{64}", record["sha256"])
    assert re.fullmatch(r"[0-9a-f]{64}", acquisition["sha256"])
    assert record["bytes"] > 0 and acquisition["artifact_bytes"] > 0
    assert record["status"] == "retrieved-artifact-roundtrip"
    assert record["operational"] is False
    assert record["run_identity_status"] == "requested_unverified"
    assert record["sampling_geometry"] == "pixel_is_area_cell_centres"
    assert record["resampling"] == "server_resampled_method_unknown"

    assert acquisition["variable"] == field.variable
    assert acquisition["run_time"] == record["run_time"]
    assert acquisition["valid_time"] == record["valid_time"]
    for stamp in (record["run_time"], record["valid_time"]):
        assert datetime.fromisoformat(stamp).tzinfo is not None
    height, width = acquisition["requested_shape"]
    assert acquisition["finite_cells"] + acquisition["nodata_cells"] == height * width
    assert acquisition["transfer_bytes"] > 0 and acquisition["elapsed_seconds"] > 0
    contract = GRID_CONTRACTS[model]
    assert contract.source_id.startswith("eccc-")


def test_live_profile_receipt_records_both_levels_without_a_payload():
    receipts = json.loads(RECEIPT_FIXTURE.read_text())
    record = receipts["representative_profile_artifact"]
    assert "retained_artifact" not in record
    assert record["pressure_levels_hpa"] == [850, 700]
    assert record["coverage_ids"] == [
        "HRDPS.CONTINENTAL.PRES_HR.850",
        "HRDPS.CONTINENTAL.PRES_HR.700",
    ]
    assert re.fullmatch(r"[0-9a-f]{64}", record["sha256"])
    assert record["bytes"] > 0
    assert record["status"] == "retrieved-profile-artifact-roundtrip"
    assert record["operational"] is False
    assert datetime.fromisoformat(record["run_time"]) < datetime.fromisoformat(record["valid_time"])


def test_receipt_fixture_carries_no_grid_payload_and_stays_readable_in_a_diff():
    assert RECEIPT_FIXTURE.stat().st_size < 20 * 1024
    text = RECEIPT_FIXTURE.read_text()
    receipts = json.loads(text)
    assert receipts["full_family_inventory_count"] == 1289
    assert len(receipts["full_family_inventory_sample"]) <= 4
    assert receipts["selected_acquisition"]["result_count"] == 245
    assert len(receipts["selected_acquisition"]["results"]) == len(REPRESENTATIVE_FIELDS)
    assert "geomet-wcs-artifacts" not in text
