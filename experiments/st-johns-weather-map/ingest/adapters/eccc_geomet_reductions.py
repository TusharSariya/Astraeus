...
SELECTED_REDUCTIONS = (
    ...
    GEPSReduction(
        "GEPS.DIAG.3_TT.ERSSTD",
        "temperature_2m__spread",
        "temperature_2m",
        "standard_deviation",
        "degC",
    ),
    GEPSReduction(
        "GEPS.DIAG.3_TT.ERC50",
        "temperature_2m__p50",
        "temperature_2m",
        "percentile",
        "degC",
    ...
    GEPSReduction(
        "GEPS.DIAG.3_NT.ERC50",
        "total_cloud_opacity__p50",
        "total_cloud_opacity",
        "percentile",
        "percent",
        quantile=0.5,
    ...
    GEPSReduction(
        "GEPS.DIAG.12_GUST-15MS.PROB",
        "raw__gust_over_15ms_probability",
        "raw_gust_threshold",
        "threshold_probability",
        "percent",
        threshold="gust > 15 m s-1",
    ),
)
...
def fetch_geps_reductions(
    *,
    ...
    if valid_time.utcoffset() is None or reference_time.utcoffset() is None:
        raise GEPSReductionError(
            "GEPS reference_time and valid_time must be offset-aware"
        )
    if valid_time < reference_time:
        raise GEPSReductionError("GEPS valid_time cannot precede reference_time")
        ...
        for reduction in reductions:
            ...
            try:
                array = decode_reps_geotiff(
                    payload,
                    coverage=reduction.coverage_id,
    ...
    dataset.attrs.update(
        {
            ...
            "provider_run_id": f"geps-{reference_time:%Y%m%dT%HZ}-f{int((valid_time - reference_time).total_seconds() // 3600):03d}",
    ...
    provenance = {
        ...
        "valid_times": [valid_time.astimezone(UTC).isoformat()],
        "provider_run_id": f"geps-{reference_time:%Y%m%dT%HZ}-f{int((valid_time - reference_time).total_seconds() // 3600):03d}",
        "retrieved_at": getattr(http, "retrieved_at", datetime.now(UTC)).isoformat(),
        ...
        "field_accounting": {
            ...
            "selected_reductions": len(reductions),
            "other_advertised_coverages_deferred": 532 - len(reductions),
        ...
        "quality": {
            "status": "unknown",
            "flags": ["experimental_source_contract_pending"],
        },
        "operational": False,
    }
