...
MAX_GZIP=2*1024*1024; MAX_DECODED=16*1024*1024; URLS={"pirep_airep":"https://aviationweather.gov/data/cache/aircraftreports.cache.csv.gz","sigmet":"https://aviationweather.gov/data/cache/airsigmets.cache.csv.gz"}
 ...
 def fetch(self,candidate,window,workdir):
  ...
  try:
   for kind,url in URLS.items():
    try:
     body,h,done=self._client.get_bytes_with_headers_completed(url,max_bytes=MAX_GZIP)
    except (MaxBytesExceeded,RetriesExhausted,httpx.HTTPError) as e:
     raise AdapterUnavailable(f'awc aviation: unavailable {kind}: {e}') from e
