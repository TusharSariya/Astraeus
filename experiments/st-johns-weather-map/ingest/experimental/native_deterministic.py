...
def _retrieval_time(candidate: RunCandidate) -> datetime:
    if not candidate.detail.get("retained_raw"):
        return datetime.now(UTC)
    original = candidate.detail.get("retained_retrieved_at")
    if not isinstance(original, datetime) or original.utcoffset() is None:
        raise AdapterUnavailable("retained evidence lacks original retrieval time")
    return original.astimezone(UTC)
...
class IndexedNativeCandidate:
    ...
    def fetch(self, candidate: RunCandidate, window: FetchWindow, workdir: Path) -> RunResult:
        ...
        if candidate.detail.get("retained_raw"):
            ...
            retained_bytes = candidate.detail.get("retained_raw_bytes")
            retained_digest = candidate.detail.get("retained_raw_sha256")
            if not isinstance(retained_bytes, int) or not isinstance(retained_digest, str):
                raise AdapterUnavailable("retained raw bundle has no immutable size and digest")
            if byte_size != retained_bytes:
                raise AdapterUnavailable(f"retained raw manifest size mismatch: wanted {retained_bytes}, got {byte_size}")
            if digest != retained_digest:
                raise AdapterUnavailable(f"retained raw bundle digest mismatch: {self.source_id}")
        else:
        ...
        retrieved_at = _retrieval_time(candidate)
        provenance = {"source_id": self.source_id, "product_access_path": candidate.urls[0], "index_url": candidate.urls[1],
            ...
            "valid_time": run.isoformat(), "retrieved_at": retrieved_at.isoformat(), "raw_sha256": digest,
        ...
        return RunResult(self.source_id, candidate.provider_run_id, run, retrieved_at, complete, True, [artifact],
...
class DWDIconNativeCandidate:
    ...
    def fetch(self, candidate: RunCandidate, window: FetchWindow, workdir: Path) -> RunResult:
        ...
        provenance = {"source_id": self.source_id, "product_access_path": DWD_BASE, "provider_run_id": candidate.provider_run_id, "run_time": run.isoformat(), "lead_hours": 0,
            "valid_time": run.isoformat(), "retrieved_at": retrieved_at.isoformat(), "raw_sha256": digest.hexdigest(), "retrieved_objects": retrieved,
        ...
        artifact = Artifact("native-deterministic", MEDIA_ZARR, artifact_path, provenance)
        return RunResult(self.source_id, candidate.provider_run_id, run, retrieved_at, True, True, [artifact], native_crs="ICON R03B07 native icosahedral", notes="unregistered experimental evidence candidate")
